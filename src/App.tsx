    ...
    <div className="w-screen h-screen bg-[#f8f9fa] overflow-hidden flex flex-col">
      ...
      {status !== 'open' && (
        <div className="absolute inset-x-0 top-14 z-40 flex justify-center pointer-events-none">
          <div className="bg-amber-500 text-white px-6 py-3 rounded-b-xl shadow-lg flex items-center gap-3 animate-pulse pointer-events-auto">
            <svg className="w-5 h-5 animate-spin" viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
            </svg>
            <span className="font-semibold">
              {status === 'connecting' ? 'Connecting to server...' : 'Connection lost. Reconnecting...'}
            </span>
          </div>
        </div>
      )}
